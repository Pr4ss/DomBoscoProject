...
def estoque_atualizado(ID_Produto):
    url = f"https://api.mercadolibre.com/items/{ID_Produto}"
    resposta = requests.get(url)
    if resposta.status_code == 200:
        dados_produto = resposta.json()
        estoque = dados_produto.get("initial_quantity", "Não disponível")
...
def atualizar_estoque(ID_Produto, novo_estoque):
    ...
    if resposta.status_code == 200:
        ...
        print(f"Não foi possível atualizar o estoque do produto ID {ID_Produto}")
        print(resposta.status_code)

def atualizar_status(ID_Produto, Status):
    url = f"https://api.mercadolibre.com/items/{ID_Produto}"
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json"
